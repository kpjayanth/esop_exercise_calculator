...
import { Label, ToggleGroup, Tooltip, Divider } from '@/components/ui/index'
...
export function InputPanel({ inputs, onChange, grants, onResetGrants, exerciseDate, onExerciseDateChange }: Props) {
  ...
  return (
    <div className="space-y-5">
      {/* Primary inputs */}
      <div>
        <p className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-wide mb-3">Exercise Details</p>
          ...
          <Field label="FMV at Exercise" tooltip="Current fair market value per share (from latest company valuation / 409A)">
            ...
            {inputs.fmvAtExercise > 0 && (
              <p className="text-[11px] text-[#9CA3AF] mt-1 pl-1">
                {toIndianWords(inputs.fmvAtExercise)}
              </p>
            )}
          </Field>
            ...
            <div className="space-y-2">
              ...
              {allocationRows.map((a) => (
                <div key={a.grantId} className="rounded-xl border border-[#E5E7EB] bg-white overflow-hidden">
                  ...
                  {/* Stats grid: Available | Selected | Shares */}
      ...
      <div>
        <p className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-wide mb-3">Tax Details</p>
        <div className="space-y-4">
          <Field label="Tax Regime" tooltip="New regime (Budget 2025 slabs) vs Old regime. We'll show both anyway.">
            <ToggleGroup
              options={[
          ...
          <Field label="Annual Salary Income" tooltip="Your total salary for the year, excluding this ESOP exercise. Used to determine your marginal tax slab.">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-7 pr-3 py-2 text-sm rounded-lg border border-[#E5E7EB] focus:outline-none focus:ring-2 focus:ring-[#E85936]/20 focus:border-[#E85936] transition-all text-[#6B7280]"
              />
            </div>
            {inputs.annualSalaryIncome > 0 && (
              <p className="text-[11px] text-[#9CA3AF] mt-1 pl-1">
                {toIndianWords(inputs.annualSalaryIncome)}
              </p>
            )}
          </Field>
        </div>
