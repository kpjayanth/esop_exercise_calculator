...
export function GrantUpload({ onGrantsLoaded }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#F6F9FB] flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg space-y-6">
        {/* Header */}
        <div className="text-center">
          <div className="w-14 h-14 rounded-lg bg-gradient-to-br from-[#E85936] to-[#f97316] flex items-center justify-center mx-auto mb-4 shadow-md">
            <FileSpreadsheet size={28} className="text-white" />
          </div>
          <h1 className="text-xl font-bold text-[#071437]">Load Your Grant Data</h1>
        ...
        {!grants && (
          ...
          >
            ...
            />
            <Upload size={32} className="mx-auto mb-3 text-[#99A1B7]" />
            <p className="text-sm font-medium text-[#252F4A]">
              Drop your Excel file here, or <span className="text-[#E85936]">browse</span>
            </p>
            <p className="text-xs text-[#99A1B7] mt-1">.xlsx · .xls · .csv</p>
            <button
              onClick={(e) => { e.stopPropagation(); loadSampleData() }}
              className="mt-3 text-xs text-[#99A1B7] hover:text-[#E85936] underline underline-offset-2 transition-colors"
            >
              Try sample data →
            </button>
          </div>
        ...
        {grants && (
          <div className="bg-white rounded-lg border border-[#F1F1F4] shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-[#F1F1F4] flex items-center gap-2">
              ...
              <span className="ml-auto text-xs text-[#99A1B7]">{totalVested.toLocaleString('en-IN')} options available</span>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[11px] font-[500] text-[#99A1B7] uppercase tracking-[0.08em] bg-[#F6F9FB] border-b border-[#F1F1F4]">
                  <th className="px-4 py-2 text-left">Grant</th>
                  <th className="px-4 py-2 text-left">Date</th>
                  <th className="px-4 py-2 text-right">Strike</th>
                  <th className="px-4 py-2 text-right">Vested</th>
                  <th className="px-4 py-2 text-right">Ratio</th>
                </tr>
              </thead>
              <tbody>
                {grants.map((g) => (
                  <tr key={g.grantId} className="border-b border-[#F1F1F4] last:border-0">
                    <td className="px-4 py-2.5 font-medium text-[#252F4A]">{g.grantId}</td>
                    <td className="px-4 py-2.5 text-[#99A1B7]">{formatGrantDate(g.dateOfGrant)}</td>
                    <td className="px-4 py-2.5 text-right text-[#252F4A]">₹{g.exercisePrice.toLocaleString('en-IN')}</td>
                    <td className="px-4 py-2.5 text-right font-semibold text-[#071437]">{g.vestedOptions.toLocaleString('en-IN')}</td>
                    <td className="px-4 py-2.5 text-right">
                      {g.conversionRatio === 1 ? (
                        <span className="text-[#99A1B7]">1 : 1</span>
                      ) : (
                        <span className="font-semibold text-[#252F4A]">{g.conversionRatio} : 1</span>
                      )}
                    </td>
                  </tr>
                ))}
            ...
            <div className="px-5 py-3 flex items-center justify-between">
              <button
                ...
                className="text-xs text-[#99A1B7] hover:text-[#252F4A] transition-colors"
              >
                ← Upload different file
              </button>
              <button
                onClick={() => onGrantsLoaded(grants)}
                className="flex items-center gap-2 h-11 px-5 rounded-[6px] bg-[#E85936] text-white text-[14px] font-[500] tracking-[0.35px] hover:bg-[#d14e2e] transition-colors"
              >
                Calculate Tax
        ...
        {/* Format guide */}
        <div className="bg-white rounded-xl border border-[#F1F1F4] px-4 py-3">
          <p className="text-xs font-semibold text-[#252F4A] mb-2">Expected Excel Format</p>
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-[#99A1B7]">
              <thead>
                <tr className="font-medium text-[#99A1B7]">
